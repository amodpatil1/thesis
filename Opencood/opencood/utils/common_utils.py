...
def torch_tensor_to_numpy(torch_tensor):
    ...
    # --- handle dicts or lists recursively ---
    if isinstance(torch_tensor, dict):
        return {k: torch_tensor_to_numpy(v) for k, v in torch_tensor.items()}
    if isinstance(torch_tensor, (list, tuple)):
        return [torch_tensor_to_numpy(v) for v in torch_tensor]

    # --- normal tensor conversion ---
    if isinstance(torch_tensor, torch.Tensor):
        return torch_tensor.detach().cpu().numpy()

    # --- fallback for scalars/arrays ---
