...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            cav_pose = np.asarray(selected_cav_base['params']['lidar_pose']).astype(float).squeeze()
            ego_pose = np.asarray(ego_lidar_pose).astype(float).squeeze()
            # handle extra dims safely
            cav_xy = cav_pose[..., :2]
            ego_xy = ego_pose[..., :2]
