...
class BaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        if ego_cav_content is None:
            # Fallback: choose the first cav as ego if nothing else
            for _cid, _cc in scenario_database.items():
                if _cc.get('ego', False):
                    ego_cav_content = _cc
                    break
            if ego_cav_content is None:
                ego_cav_content = next(iter(scenario_database.values()))
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        """
        def pick_valid_ts(cav_content, preferred):
            # Use preferred timestamp if present, else first real timestamp key
            if preferred in cav_content:
                return preferred
            keys = [k for k in cav_content.keys()
                    if k != 'ego' and isinstance(cav_content[k], dict) and 'yaml' in cav_content[k]]
        ...
        for _cid, _cc in scenario_database.items():
            if _cc.get('ego', False):
                ego_cav_content = _cc
                ego_ts = pick_valid_ts(_cc, timestamp_key)
                break

        if ego_cav_content is None or ego_ts is None:
            return None  # caller will handle fallback

        ego_lidar_pose = load_yaml(ego_cav_content[ego_ts]['yaml'])['lidar_pose']
        ego_xy = np.asarray(ego_lidar_pose[:2], dtype=float).reshape(-1)

        # ---- compute distances for all cavs (each picks a valid ts)
        for cav_id, cav_content in scenario_database.items():
            cav_ts = pick_valid_ts(cav_content, timestamp_key)
            if cav_ts is None:
                continue
            cur_lidar_pose = load_yaml(cav_content[cav_ts]['yaml'])['lidar_pose']
            cur_xy = np.asarray(cur_lidar_pose[:2], dtype=float).reshape(-1)
            diff = cur_xy - ego_xy
        ...
        # # robust to list/tuple/ndarray poses
        # ego_xy = np.asarray(ego_lidar_pose[:2], dtype=float).reshape(-1)
        # cur_xy = np.asarray(cur_lidar_pose[:2], dtype=float).reshape(-1)
